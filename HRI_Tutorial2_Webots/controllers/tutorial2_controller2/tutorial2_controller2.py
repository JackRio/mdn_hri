...
class MyRobot(Robot):
    ...
    def predict_movement(self,x,y, is_MDN):
        # load model 
        ...
        # if using FFN
            ...
            prediction = ffnmodel.forward(input_data)
            prediction = prediction.data.numpy()
            return prediction[0,0], prediction[0,1]
    ...
    def follow_ball(self, is_MDN=True):
        ...
        while self.step(self.timeStep) != -1:
             ...
             if x_ball != None:
                 ...
                 pitch_1, roll_1 = self.predict_movement(x_ball,y_ball,is_MDN)
                 pitch_2, roll_2 = self.predict_movement(x_ball,y_ball,is_MDN)
                 pitch_3, roll_3 = self.predict_movement(x_ball,y_ball,is_MDN)
                 pitch_4, roll_4 = self.predict_movement(x_ball,y_ball,is_MDN)

                 new_pitch,new_roll = (pitch_1+pitch_2 + pitch_3+ pitch_4)/4, (roll_1 +roll_2+ roll_3+ roll_4)/4
                 roll_position = (old_roll + new_roll)/2
                 pitch_position = (old_pitch + new_pitch)/2
                 ...
                 old_pitch = pitch_position
                 old_roll = roll_position
